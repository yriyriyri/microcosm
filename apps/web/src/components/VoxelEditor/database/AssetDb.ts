...
import type { AssetCompiledRender } from "../domain/compiledAssetTypes";
...
export async function saveAsset(params: {
  ...
  const data: AssetData = {
    id,
    group: params.group,
    compiledRender: buildAssetCompiledRender(params.group),
  };
...
export async function saveAssetRecord(record: AssetStoredRecord): Promise<string> {
  const db = await openDb();
...
export async function loadAssetRecord(
  id: string
): Promise<AssetStoredRecord | null> {
  const db = await openDb();
