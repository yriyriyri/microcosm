...
type GroupRecord = {
  ...
  source: GroupSource;
  compiledRender: AssetCompiledRender | null;
  compiledVisualMeshes: THREE.Mesh[];
  compiledPickMesh: THREE.Mesh | null;
};
...
export type PublishedWorldSurface = AssetCompiledSurface;
...
export class VoxelWorld {
  ...
  private disposeMeshOnly(mesh: THREE.Mesh | null, opts?: { disposeMaterial?: boolean }) {
    if (!mesh) return;
  ...
  private clearCompiledArtifacts(group: GroupRecord) {
    ...
    if (group.compiledPickMesh) {
      this.disposeMeshOnly(group.compiledPickMesh, { disposeMaterial: true });
      group.compiledPickMesh = null;
    }
  }

  private disposeGroup(group: GroupRecord) {
  ...
  private applyGroupRootUserData(groupId: GroupId, group: GroupRecord) {
    ...
    group.root.userData.renderAssetId =
      group.source.overrideAssetId ?? group.source.assetId ?? null;
  ...
  private buildPickMeshFromCompiledRender(
    groupId: GroupId,
    source: GroupSource,
    ...
    (material as THREE.MeshBasicMaterial & { colorWrite?: boolean }).colorWrite = false;
  ...
  private syncCompiledGroupRender(groupId: GroupId, group: GroupRecord) {
    ...
    const compiledRender =
      group.compiledRender ??
      (snapshot ? buildAssetCompiledRender(snapshot) : null);
    ...
    const pickMesh = this.buildPickMeshFromCompiledRender(
      groupId,
      group.source,
      compiledRender
    );
  ...
  setGroupVoxelsLocal(
    groupId: GroupId,
    voxels: GroupVoxel[],
    opts?: { keepPosition?: boolean; compiledRender?: AssetCompiledRender | null }
  ): boolean {
  ...
  setGroupSource(groupId: GroupId, source: Partial<GroupSource>): boolean {
    ...
    for (const mesh of g.compiledVisualMeshes) {
      ...
      mesh.userData.renderAssetId =
        g.source.overrideAssetId ?? g.source.assetId ?? null;
  ...
  setGroupId(coord: VoxelCoord, nextGroupId: GroupId) {
    ...
    if (this.renderCfg.mode === "compiled") {
      ...
      this.syncCompiledGroupRender(from.root.userData.groupId as string, from);
    }
  }
  ...
  private transformCompiledBoundsToWorld(
    ...
    const matrix = new THREE.Matrix4()
      .makeRotationFromEuler(rotationToEuler(rotation))
      .setPosition(position.x, position.y, position.z);

    const box = new THREE.Box3();
    const corners = [
      new THREE.Vector3(bounds.min.x, bounds.min.y, bounds.min.z),
      new THREE.Vector3(bounds.min.x, bounds.min.y, bounds.max.z + 1),
      new THREE.Vector3(bounds.min.x, bounds.max.y + 1, bounds.min.z),
      new THREE.Vector3(bounds.min.x, bounds.max.y + 1, bounds.max.z + 1),
      new THREE.Vector3(bounds.max.x + 1, bounds.min.y, bounds.min.z),
      new THREE.Vector3(bounds.max.x + 1, bounds.min.y, bounds.max.z + 1),
      new THREE.Vector3(bounds.max.x + 1, bounds.max.y + 1, bounds.min.z),
      new THREE.Vector3(bounds.max.x + 1, bounds.max.y + 1, bounds.max.z + 1),
    ];
  ...
  private async getPublishedBakedGroupSnapshot(
    ...
    const compiledRender = loaded.compiledRender ?? buildAssetCompiledRender(loaded.group);
    if (!compiledRender.surfaces.length) return null;
    ...
    return {
      ...
      position: { ...g.position },
      rotation: { ...g.rotation },
      bounds: this.transformCompiledBoundsToWorld(
        compiledRender.bounds,
        g.position,
        g.rotation
      ),
  ...
  private async getLatestMarketplaceAssetIdForGroup(
    ...
    try {
      ...
      const linkedMarketplaceAssetId = (meta as any)?.linkedMarketplaceAssetId;

      if (
        typeof linkedMarketplaceAssetId === "string" &&
        linkedMarketplaceAssetId.trim()
      ) {
